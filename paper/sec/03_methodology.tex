...
\textbf{ResNet.} We use a ResNet-18 variant with three residual stages. The base channel width $c$ is varied across seven levels: $c \in \{4, 8, 16, 24, 32, 48, 64\}$, yielding parameter counts spanning approximately 11K to 2.78M. Skip connections and batch normalization are standard throughout. This architecture is the primary vehicle for studying double descent, as convolutional architectures with residual connections are where the phenomenon is expected to manifest most clearly~\cite{nakkiran2020deep}.
...
To assess whether the MixUp effect depends on interpolation strength, we repeat the MLP/CIFAR-10 experiment with five values of $\alpha$: $\{0.1, 0.2, 0.4, 0.8, 1.0\}$. We measure test error at each model width and compare curves across $\alpha$ values.
